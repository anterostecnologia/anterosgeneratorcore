...
public class GenerationMvcConfigurationStrategy implements AnterosGenerationStrategy {
	...
	public void generate(AnterosGenerationConfig config) throws Exception {
		...
		if (SQL.equals(config.getPersistenceDatabase())){
			templateMvc = config.getConfiguration().getTemplate(SQL + File.separatorChar +MVC_CONFIGURATION_TEMPLATE);
		}
		else {
			templateMvc = config.getConfiguration().getTemplate(NO_SQL+ File.separatorChar +MVC_CONFIGURATION_TEMPLATE);
		}

...
public class GenerationResourcePersistenceConfigurationStrategy implements AnterosGenerationStrategy {
	...
	public void generate(AnterosGenerationConfig config) throws Exception {
		...
		if (SQL.equals(config.getPersistenceDatabase())){
			templatePersistence = config.getConfiguration().getTemplate(SQL + File.separator +RESOURCE_PERSISTENCE_CONFIGURATION_TEMPLATE);
		}
		else {
		 templatePersistence = config.getConfiguration().getTemplate(NO_SQL+ File.separator +RESOURCE_PERSISTENCE_CONFIGURATION_TEMPLATE);
		}
		
		
		dataModel = new HashMap<String, Object>();
		File persistenceFile = new File(config.getPackageDirectory() + File.separator + "config"
				+ File.separator + "ResourcePersistenceConfiguration.java");

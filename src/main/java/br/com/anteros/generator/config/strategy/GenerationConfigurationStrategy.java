	...
	public void generate(AnterosGenerationConfig config) throws Exception {
		...
		if (!mvcFile.exists()) {
			...
			if (config.isGenerateSwaggerConfiguration()) {
				...
				dataModel.put(IMPORT_SWAGGER,
						"import " + config.getPackageDestination() + ".config.doc.SwaggerConfiguration;");
			} else {
			...
			if (config.isGenerateJSONDocConfiguration()) {
				...
				dataModel.put(IMPORT_JSON_DOC,
						"import " + config.getPackageDestination() + ".config.doc.JSONDocConfiguration;");
			} else {
		...
		if (config.isGenerateSwaggerConfiguration()) {
			FileUtils.forceMkdir(new File(
					config.getPackageDirectory() + File.separatorChar + "config" + File.separatorChar + "doc"));
			Template templateSwagger = config.getConfiguration().getTemplate(SWAGGER_CONFIGURATION_TEMPLATE);
			dataModel = new HashMap<String, Object>();
			File swaggerFile = new File(config.getPackageDirectory() + File.separatorChar + "config"
					+ File.separatorChar + "doc" + File.separatorChar + "SwaggerConfiguration.java");
		...
		if (config.isGenerateJSONDocConfiguration()) {
			...
			dataModel = new HashMap<String, Object>();
			File JSONDocFile = new File(config.getPackageDirectory() + File.separatorChar + "config"
					+ File.separatorChar + "doc" + File.separatorChar + "JSONDocConfiguration.java");

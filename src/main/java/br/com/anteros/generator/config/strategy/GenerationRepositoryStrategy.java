...
public class GenerationRepositoryStrategy implements AnterosGenerationStrategy {
	...
	public void generate(AnterosGenerationConfig config) throws Exception {
		...
		if (SQL.equals(config.getPersistenceDatabase())){
			templateServiceInterface = config.getConfiguration().getTemplate(SQL + File.separator +REPOSITORY_INTERFACE_TEMPLATE);
		}
		else {
		 templateServiceInterface = config.getConfiguration().getTemplate(NO_SQL+ File.separator +REPOSITORY_INTERFACE_TEMPLATE);
		}
		...
		File fileService = new File(config.getSourceDestination()+ File.separator + sourcePackageName.replace('.', File.separatorChar)+File.separator+ config.getClazz().getName() + "Repository.java");
		...
		if (SQL.equals(config.getPersistenceDatabase())){
			templateServiceImpl = config.getConfiguration().getTemplate(SQL + File.separator +REPOSITORY_IMPLEMENTATION_TEMPLATE);
		}
		else {
			templateServiceImpl = config.getConfiguration().getTemplate(NO_SQL+ File.separator +REPOSITORY_IMPLEMENTATION_TEMPLATE);
		}


		dataModel = new HashMap<String, Object>();
		
		FileUtils.forceMkdir(new File(config.getSourceDestination()+ File.separator + sourcePackageName.replace('.', File.separatorChar)+File.separator));
		
		File fileServiceImpl = new File(config.getSourceDestination()+ File.separator + sourcePackageName.replace('.', File.separatorChar)+File.separator+ config.getClazz().getName() + "RepositoryImpl.java");
